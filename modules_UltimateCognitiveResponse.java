...
public class VhalinorPredictorModule {
    ...
    public static class Predictor {
        ...
        protected static boolean PANDAS_AVAILABLE = false;
        ...
        protected double calculateVolatility(List<Double> priceList, int index) {
            ...
            double mean = slice.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
            double var = slice.stream().mapToDouble(p -> Math.pow(p - mean, 2)).average().orElse(0.0);
        ...
        protected double calculateTrend(List<Double> priceList, int index) {
            ...
            double avg = priceList.subList(index - 10, index).stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
        ...
        protected double calculateBollingerUpper(List<Double> priceList, int index) {
            ...
            double std = Math.sqrt(slice.stream().mapToDouble(p -> Math.pow(p - sma, 2)).average().orElse(0.0));
            return sma + 2 * std;
        }
    ...
    public static void main(String[] args) {
        ...
        ensemblePredictor.predictWithDynamicEnsemble("ETHUSD", sampleFeatures, "1h")
                .thenAccept(r -> System.out.println("Dynamic Prediction: " + r.getPrediction() + " Confidence: " + r.getConfidence()))
                .join();
    }
}